...
/** Is a generated type expression already an array or tuple type? */
function isArrayLikeType(expression: TSNode): boolean {
  return expression.endsWith("[]") || (expression.startsWith("[") && expression.endsWith("]"));
}
  ...
  if ("type" in schemaObject && schemaObject.type) {
    ...
    if (schemaObject.type === "array") {
      ...
      const buildItemType = (elementIndent: string): TSNode => {
        ...
        if (schemaObject.items) {
          if (hasKey(schemaObject.items, "type") && schemaObject.items.type === "array") {
            return `${tsParenthesize(transformSchemaObject(schemaObject.items, options, false, elementIndent))}[]`;
          }
      ...
      if (
        ...
        elements.push(`...${tsParenthesize(itemType)}[]`);
        return tupleType(elements, indent);
      }

      const itemType = buildItemType(indent);
      const finalType = isTupleShape || isArrayLikeType(itemType) ? itemType : `${tsParenthesize(itemType)}[]`;
      ...
      for (const [k, v] of getEntries(schemaObject.properties ?? {}, options.ctx)) {
        ...
        coreObjectType.push(
          propertySignature({
            ...
            /* modifiers     */ readonly: options.ctx.immutable || (!options.ctx.readWriteMarkers && readOnly),
      ...
      for (const [k, v] of Object.entries(schemaObject.$defs)) {
        ...
        defKeys.push(
          propertySignature({
            ...
            type: finalProperty.type,
            optional: finalProperty.optional,
            /* modifiers    */ readonly: options.ctx.immutable || (!options.ctx.readWriteMarkers && defReadOnly),
