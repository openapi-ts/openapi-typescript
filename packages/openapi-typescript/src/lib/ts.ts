 ...
 *
 * Everything reproduces byte-for-byte what the TypeScript compiler printer used
 * to emit, so this package no longer needs the TypeScript compiler API at runtime.
 */
 ...
 */
function needsParentheses(expression: TSNode): boolean {
  let depth = 0;
  for (let i = 0; i < expression.length; i++) {
    ...
    if (ch === "/" && expression[i + 1] === "/") {
      ...
      i = end === -1 ? expression.length - 1 : end;
      continue;
    }
    if (ch === "=" && expression[i + 1] === ">") {
...
export function tsArrayLiteralExpression(
  ...
  if (options?.injectFooter && !footerIncludes(options.injectFooter, "type FlattenedDeepRequired<")) {
...
const HELPER_FLATTENED_DEEP_REQUIRED =
...
function footerIncludes(injectFooter: FooterDeclaration[], needle: string): boolean {
  return injectFooter.some((declaration) => typeof declaration === "string" && declaration.includes(needle));
}
...
export function tsWithRequired(
  ...
  if (!footerIncludes(injectFooter, "type WithRequired<")) {
...
export function tsReadonlyArray(type: TSNode, injectFooter?: FooterDeclaration[]): TSNode {
  if (injectFooter && !footerIncludes(injectFooter, "type ReadonlyArray<")) {
 ...
 * declarations and guarantees exactly one trailing newline. It remains exported
 * for compatibility with code written against the AST-based API.
 */
export function astToString(ast: TSNode | TSNode[], _options?: AstToStringOptions): string {
