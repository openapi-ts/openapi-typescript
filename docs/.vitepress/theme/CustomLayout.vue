...
<template>
  <Layout>
    <template #layout-top
      ><a class="banner" href="https://zuplo.com/" target="_blank">
        <svg
          class="banner-logo"
        ...
        >
        <span class="banner-pseudo-link">Learn more →</span>
      </a></template
    >
  </Layout>
</template>
...
.banner {
  --zuplo-brand: #ff00bd;
  --zuplo-brand-a11y: #eb08ae;

  background-color: var(--zuplo-brand-a11y);
  color: #fff;
  display: flex;
  font-size: 0.75rem;
  font-weight: 500;
...
.banner:hover .banner-pseudo-link {
...
:root {
  --banner-height: 2rem;
  --vp-layout-top-height: 2rem; /* Vitepress’ variable that allows for banners */
}
